...
/*==========================[ début programme ]================================*/
    ...
    function getCLientColumn(){
        ...
        col.push({ data: 'Contacts', title: "Prénom", render: renderContactName });
        col.push({ data: 'Contacts', title: "Nom", render: renderContactLastName });
    ...
    function renderContactName(data, type, row) {
        if (data.length > 0){
            var contact = $.grep(data, function (elt, i) {
                return elt.IsPrincipal;
            });
            if(contact.length > 0)
                return contact[0].FirstName;
            else
    ...
    function renderContactLastName(data, type, row) {
        if (data.length > 0) {
            var contact = $.grep(data, function (elt, i) {
                return elt.IsPrincipal;
            });
            if (contact.length > 0)
                return contact[0].LastName;
            else
